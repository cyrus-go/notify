...
const (
	...
	contextNotifyBalanceWarning    = "【端脑云】温馨提示：您正在使用的 AI 应用即将到期，系统显示您的余额不足以进行自动续费，如需继续使用请及时充值噢！到期后应用会自动停用并清除现有设置，为了方便您的使用，请确保在服务到期前进行充值或备份数据。\n    端脑云感谢您的使用，祝您生活愉快！"
	contextNotifyAppExpiredWarning = "【端脑云】温馨提示：您正在使用的 AI 应用即将到期，如需继续使用请及时续费噢！到期后应用会自动停用并清除现有设置，为了方便您的使用，请确保在服务到期前进行续费或备份数据。\n    端脑云感谢您的使用，祝您生活愉快！"
)
